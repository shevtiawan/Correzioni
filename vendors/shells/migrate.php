...
class MigrateShell extends Shell {
    ...
    */
    ...
    /**
    * Crete migration file from exist db
    */
      ...
      $this->out( 'Please wait for creating '.count($aAllowedTables).' file(s) migration ...' );
      $this->out('');
       
      foreach($aAllowedTables as $table){
        $counter++;

        $oFile = new File( MIGRATIONS_PATH. DS . $counter . '_'.$table.'.yml', true );

        if( !$oFile->writable() ){
            $this->out( '' );
            $this->out( 'Your config/migrations folder is not writable. Please check your permissions.' );
            $this->out('');
            exit;
        }
        $oFile->write( $this->oMigrations->create_migration_file($table) );
      }

        $this->out('Done!! Created new migration file(s)) : '.(count($aAllowedTables)-count($failed)));
        $this->out('Failed created file(s)  : '.count($failed));
        if(count($failed) > 0){
           $this->out('Failed list : '.implode(", ",$failed));
        }
		}
    ...
    */
    function help(){
        ...
        $this->out('');
        $this->out('  cake migrate export_db');
        $this->out('    - Create migration file from exist database.');
        $this->out('');
