...
class Migrations{
    ...
    */
    ...
    function create_migration_file($table){
        $aResult = array();
        $aResult['UP'] = $aResult['DOWN'] = array();
        $aResult['UP']['create_table'] = $aResult['DOWN']['drop_table'] = array();

        $columns = $this->oDb->describe($table);
        $aResult['UP']['create_table'][$table] = array();

        foreach( $columns as $column => $data ){
          if($column == 'id' || $column == 'created' || $column == 'modified')
          continue;

          $aResult['UP']['create_table'][$table][$column] = array();
          foreach($data as $key => $value){
            $aResult['UP']['create_table'][$table][$column][$key] = $value;
          }
        }
