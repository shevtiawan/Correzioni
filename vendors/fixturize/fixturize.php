...
class Fixturize{
    ...
    * @return void
    */
    function Fixturize($sConnection = 'default'){
    ...
    function table_to_fixture_file($table, $records)
    {        
        foreach($records as $key=>$record){
          $key = ''+$key+1;
          $aResult[$table][$key] = array();
          foreach($record['Model'] as $column_name => $value){
            $aResult[$table][$key][$column_name] = $value;
           }
        }
