...
class AcosController extends AppController {
    ...
    function admin_index($aco_parent_id = null) {
        if (empty($aco_parent_id)) {
            $aco_parent_id = 1;
        } else {
            $this->Aco->actsAs[] = 'Tree';
            $path = $this->_getAcoPathList($aco_parent_id);
        }
        ...
        $acos = $this->Aco->find('all', array('order' => 'lft', 'conditions' => array('parent_id' => $aco_parent_id)));
        $this->set(compact('acos', 'path', 'actionpath', 'aco_parent_id'));
    }

    function admin_add($aco_parent_id = null) {
        if (!empty($this->data)) {
            if (empty($this->data['Aco']['parent_id'])) {
                $this->data['Aco']['parent_id'] = 1;//$this->root_id;
            }
            ...
            $this->data['Aco']['parent_id'] = $aco_parent_id;
        }
